...
const EpicCard: React.FC<EpicCardProps> = ({ 
  ...
  return (
    <div className="bg-white/10 backdrop-blur-lg border border-white/20 rounded-xl overflow-hidden hover:bg-white/15 transition-all duration-200 animate-fade-in">
      {/* Header de la épica (siempre visible) */}
      <div className="p-6">
        <div className="flex items-start justify-between mb-4">
          <div className="flex items-start space-x-4 flex-1">
            <div className="w-12 h-12 bg-gradient-to-r from-[#0EA5E9] to-[#0264C5] rounded-lg flex items-center justify-center text-white ring-2 ring-[#FFCD00]/30">
              {getEpicIcon(epic.status)}
            </div>
            
            <div className="flex-1 min-w-0">
              <div className="flex items-center space-x-3 mb-2">
                <h3 className="text-xl font-semibold text-white line-clamp-1">{epic.title}</h3>
              ...
              <p className="text-blue-200 text-sm line-clamp-2 mb-3">
                {epic.description || 'Sin descripción disponible'}
              </p>
              ...
              <div className="flex items-center space-x-6 text-sm text-blue-300">
                <div className="flex items-center space-x-1">
                  ...
                  <span>{epic._count?.userStories || 0} historias</span>
                </div>
                ...
                {isExpanded && (
                  <div className="flex items-center space-x-1">
                    ...
                    <span>{getTotalStoryPoints()} puntos</span>
                  </div>
                )}
                
                <div className="text-xs">
                  Creado: {formatDate(epic.createdAt)}
                </div>
              </div>
            </div>
          </div>
        ...
        {epic.businessValue && (
          <div className="flex items-center space-x-2 text-sm">
            ...
            <span className="text-blue-200">Valor de negocio:</span>
            <span className="text-yellow-300 font-medium">{epic.businessValue}</span>
          </div>
        )}
      </div>
        ...
        <div className="border-t border-white/20 p-6 bg-white/5">
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <h4 className="text-lg font-medium text-white">Historias de Usuario Relacionadas</h4>
              <button className="bg-green-600 hover:bg-green-700 text-white px-3 py-1 rounded-lg transition-colors text-sm flex items-center space-x-1">
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 6v6m0 0v6m0-6h6m-6 0H6" />
                </svg>
            ...
            {isLoadingStories ? (
              <div className="text-center py-8">
                ...
                <p className="text-blue-200 text-sm">Cargando historias...</p>
              </div>
              ...
              <div className="text-center py-8">
                ...
                <p className="text-blue-200 text-sm mb-3">No hay historias de usuario en esta épica</p>
              ...
              <div className="space-y-3">
                {userStories.map((story) => (
                  <div key={story.id} className="bg-white/5 border border-white/10 rounded-lg p-4 hover:bg-white/10 transition-colors">
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <div className="flex items-center space-x-3 mb-2">
                          <h5 className="text-white font-medium line-clamp-1">{story.title}</h5>
                        ...
                        <p className="text-blue-200 text-sm line-clamp-2">{story.description}</p>
                      ...
                      <div className="flex items-center space-x-2 ml-4">
                        <button className="text-blue-300 hover:text-white transition-colors p-1 rounded hover:bg-blue-500/20">
                          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            {epic.associatedSprints && epic.associatedSprints.length > 0 && (
              <div className="pt-4 border-t border-white/10">
                <h4 className="text-lg font-medium text-white mb-3">Sprints Relacionados</h4>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {epic.associatedSprints.map((sprint) => (
                    <div key={sprint.id} className="bg-white/5 border border-white/10 rounded-lg p-3">
                      <div className="flex items-center justify-between">
                        <div>
                          <h5 className="text-white font-medium">{sprint.name}</h5>
                          <div className="text-xs text-blue-300 mt-1">
                            {sprint.userStories.length} historias • {sprint.totalStoryPoints} puntos
                          </div>
                        </div>
